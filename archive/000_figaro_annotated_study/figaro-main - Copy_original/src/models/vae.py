...
# Implementation adapted from VQ-VAE2 (rosinality) and OpenAI Jukebox
...
class VectorQuantizeEMA(nn.Module):
    def __init__(self, d_latent, n_codes, n_groups=1, decay=0.995, eps=1e-4, restart_threshold=0.99):
        # Ensure latent dim is divisible by number of groups (needed for splitting)
        ...
        self.d_latent = d_latent # Total latent dimension
        self.n_groups = n_groups # Number of independent code groups
        self.dim = d_latent // n_groups # Latent size per group
        self.n_codes = n_codes # Number of codes per group

        self.decay = decay # EMA decay rate
        self.eps = eps # Small constant for numerical stability
        self.threshold = restart_threshold # Threshold to trigger reinitialization
        self.init = False # Flag to track whether centroids are initialized
        # === Embedding table ===
        embed = torch.randn(self.n_codes, self.dim) # Randomly initialized codebook
        self.register_buffer('embedding', embed) # Shape: [n_codes, dim]
        self.register_buffer('cluster_size', torch.ones(self.n_codes)) # Tracks how many points are assigned to each centroid
        self.register_buffer('cluster_sum', embed.clone().detach()) # Accumulates sum of assignments for EMA

    def forward(self, x, dist=None):
        # x: [batch, ..., d_latent], must match expected total dim
        ...
        x_ = x.reshape(-1, self.dim) # Flatten batch and group axes → [batch * groups, dim]

         # === Lazy initialization of centroids (only once) ===
        ...
        diff = (quantize.detach() - x).pow(2).mean() # Distance between x and quantized x (no gradients to quantize)
        # === Straight-through estimator ===
        quantize = x + (quantize - x).detach() # Replace x with quantized version in forward pass only
        # === Final code indices reshaped to [batch, n_groups] ===
    ...
    def _init_embeddings(self, x, dist=None):
        self.init = True
        # === Choose initial cluster centers randomly from input vectors ===
    ...
    def _randomize(self, x):
        n = x.size(0) # Number of vectors available in the batch
        if n < self.n_codes:
            # Not enough vectors to initialize all codebook entries → replicate and jitter
            r = (self.n_codes + n - 1) // n  # Minimum replication factor to reach n_codes
            std = 0.01 / np.sqrt(self.dim) # Small std dev to avoid duplicates
            x = x.repeat(r, 1) # Repeat x so it has enough rows
            x += std * torch.randn_like(x)  # Add Gaussian noise
        # Randomly shuffle and pick the first n_codes vectors
        return x[torch.randperm(x.size(0))][:self.n_codes]

    def _ema_update(self, x, cluster_assign, dist=None):
        with torch.no_grad(): # No gradient tracking, this is a manual update
            cluster_size = cluster_assign.sum(0)
            cluster_sum = cluster_assign.t() @ x
            
            # === Step 2: Prepare fallback random centers ===
            rand_centers = self._randomize(x)
...
class VqVaeModule(pl.LightningModule):
    def __init__(self, 
        ...
        # Enable decoder mode with cross-attention
        ...
        # === Reconstruction loss (token-level classification) ===
