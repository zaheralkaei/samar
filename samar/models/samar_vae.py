...
class VqEmbeddingEMA(nn.Module):
    ...
    def forward(self, z):
        ...
        distances = (flat_z ** 2).sum(1, keepdim=True) - 2 * flat_z @ self.embedding.T + (self.embedding ** 2).sum(1)
...
class SamarVQVAE(nn.Module):
    def __init__(self, d_model=128, n_embed=512, vocab_size=None, lr=3e-4):
        super().__init__()
